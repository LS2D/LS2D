...
def _download_cams_file(settings, variables, grid):
    ...
    """
    logger.info(f'Downloading CAMS (ADS) for {settings["date"]:%Y-%m-%d} - {settings["ftype"]}')
    ...
    if 'url_ads' not in credentials.keys() or 'key_ads' not in credentials:
        msg = 'You need to specify `url_ads` and `key_ads` in your `.cdsapirc` file!'
        logger.error(msg)
...
def download_cams(settings, variables, grid=None):
    ...
    if settings['data_source'] != 'CDS':
        msg = 'CAMS downloads only support CDS (for now...)!'
        logger.error(msg)
        raise ValueError(msg)

    if 'cdsapirc' not in settings.keys():
        msg = 'You need to specify the location of your `.cdsapirc` file in `settings`!'
        logger.error(msg)
        raise ValueError(msg)

    logger.info(f'Downloading CAMS (ADS) for period: {settings["start_date"]} to {settings["end_date"]}')

    # Check if output directory exists, and ends with '/'
    if not os.path.isdir(settings['cams_path']):
        msg = 'Output directory "{}" does not exist!'.format(settings['cams_path'])
        logger.error(msg)
    ...
    if cdsapi is None:
        msg = 'CDS API is not installed. See: https://cds.climate.copernicus.eu/api-how-to'
        logger.error(msg)
