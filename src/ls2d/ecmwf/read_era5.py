...
class Read_era5:
    ...
    def open_netcdf_files(self):
        ...
        """
        logger.info(f'Reading ERA5 (CDS/MARS) for period: {self.start} to {self.end}')
        ...
        def check_files(files):
            ...
            for f in files:
                if not os.path.exists(f):
                    logger.error('File "{}" does not exist...'.format(f))
                    file_missing = True
            return file_missing
        ...
        if files_missing:
            msg = 'One or more required ERA5 files are missing..'
            logger.error(msg)
    ...
    def read_data(self):
        ...
        if not np.all(self.time == time_check):
            ...
            msg = 'Model level and surface times are not synced!'
            logger.error(msg)
    ...
    def calculate_forcings(self, n_av=0, method='4th'):
        ...
        logger.info(f'Averaging ERA5 over a {self.area} spatial area.')
    ...
    def get_les_input(self, z):
        ...
        for var in variables.keys():
            ...
            if hasattr(self, var_era5):
                ...
                msg = 'Can\'t interpolate variable "{}"...'.format(var)
                logger.error(msg)
                raise RuntimeError(msg)
